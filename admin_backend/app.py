...
class User(db.Model):
    ...
    Auth_Status = db.Column(db.Integer, default=0, comment="实名审核状态: 0未审 1通过 2驳回")
...
def audit_task(task_id):
    ...
    try:
        ...
        return jsonify({
            "code": 20000,
            "message": message,
            "data": {"Task_ID": task.Task_ID, "Task_Status": task.Task_Status}
        })
...
if __name__ == '__main__':
    # 队友只需在此处引入他们写的蓝图(Blueprint)即可完成接口合并
